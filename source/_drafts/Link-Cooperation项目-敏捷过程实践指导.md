...
| Doing           | 正在执行的任务 | 具体到团队个人的Todo列表中，**由成员从Sprint Task中自主挑选** |
...
我们在规划上采用**多层级规划方法**，在开发和运维上采用**蜂拥式（all-at-once，Swarming）**的运作方式。

#### 产品战略规划

这主要发生在产品的前期，团队以战略视角根据市场、业务等因素，来分析产品的可行性、目标用户、目标平台、前进方向等方面，主要以团队个人在白板上进行头脑风暴的方式进行记录。

> 具体执行方法，我们之后讨论过之后再进行补充。

#### 产品组合规划

一个产品可能会分割多个产品，它们组合在一起成为一个完整的产品组合，我们可以在这一步对产品进行拆分，划分出多个产品，给它们划分优先等级，并**为每个产品产生一个初始的产品列表**。
...
我们要使用**敏捷结合OKR**，这其实建立了一个**大小迭代模型**，即**OKR**是一般以一个季度为周期的**大迭代**，敏捷使用的**冲刺**是一般以周为单位的**小迭代**。

我们的**Product Backlog**或**Version Backlog**将于OKR中的**KR（Key Results）**建立映射，而**Sprint Todo**将与OKR中的**Todo**建立映射。

我们在**每个季度**需要进行**季度规划**，对各个产品的**Product Backlog**或**Version Backlog**进行梳理
...
进度的跟踪我们使用燃起图[^Burnup Chart]或者燃尽图[^Burndown Chart]来可视化管理，具体可以落实到整个产品、每个版本和每次的冲刺。
