...
---
# 前要
&emsp;以下包含一些平时经常会用到但容易忘记的操作，或一些有记录价值的操作，但不包含那些更基本的命令或操作，一些较新版本的命令也不记录。部分存在比较官方的文档进行讲解的操作或命令，为了保持文章简短，会给出文档的链接，这里不会再展开。
...
## 配置全局用户名和邮箱
&emsp;git的当前用户的主配置文件在（在用户目录的.gitconfig文件）
``` bash
...
## 配置crlf检查处理
&emsp;core.safecrlf结合core.autocrlf配置使用，配置为warn出现告警但是仍然允许提交（默认配置），配置为true（直接Fatal拒绝提交），false（不做任何提示，但是仍然会做转换）。
``` bash
git config --global core.safecrlf warn
```

## 冗余空白检查
&emsp;在本地使用git diff等命令的时候将无意义的空白加亮，但是不会做自动修复
``` bash
git config --global core.whitespace "trailing-space,space-before-tab,tab-in-indent,tabwidth=4
```

## 生成本机的ssh密钥
&emsp;生成的密钥在用户目录的.ssh文件夹（包括公钥和私钥），需要在远程git仓库托管服务的配置中添加生成的公钥。
``` bash
...
&emsp;对于windows使用mingw终端可以直接修改终端窗口的配置
``` bash
Options->Text
Locale修改为zh_CN
...
## windows下的图形化git管理工具
&emsp;windows下推荐使用TortoiseGit
``` url
https://tortoisegit.org/
```

## 建立一个基于CGI脚本生成网页的简易项目查看器
&emsp;gitweb依赖一个web服务器，这里使用的是lighttpd。在linux环境下会比较方便，但是需要注意一些权限分配的问题。而在windows下需要修改一些仓库下的配置。
### 几个基本的命令
#### 初始化并启动gitweb
``` bash
git instaweb
...
git instaweb
```
&emsp;如果在windows上，这时候去访问gitweb的页面是不正常的，需要修改<repo path>/.git/gitweb内的配置。

#### 3.修改本地仓库gitweb的配置
&emsp;所有配置的修改都要在调用完git instaweb之后进行，并且之后要用git instaweb start来启动gitweb，而不能是git instaweb，不然会把修改过的配置重新替换掉。修改前先把lighttpd停止掉，如果git instaweb stop无法停止，直接在任务管理器上结束掉lighttpd。
...
#### 6. 安装CGI.pm模块
&emsp;这个是perl的CGI支持模块([点击下载](https://github.com/leejo/CGI.pm)),解压到<git install path>/usr/lib/perl5/vendor_perl下。
...
#### 补充遗漏的文件
&emsp;如果已经commit了，但是还存在遗漏的文件没有add，可以用下面两种方法补充进去（推荐第一种做法）。
...
### .gitignore文件
&emsp;把需要忽略掉的文件的路径/文件名加入.gitignore中（支持正则表达式），关于格式可以参考[git Documentation](https://git-scm.com/docs/gitignore)。
...
## 文件修改撤销
&emsp;**==注意：checkout相比reset更加不可逆一些，checkout操作需要慎重，不然原本的修改可能很难找得回来，但是reset会保留或者可以恢复。==**

### 撤销未进入过暂存区的修改，恢复至版本库
<center>

![状态预览](https://note.youdao.com/yws/public/resource/6535547d10d104ba71d1bca109828f7c/xmlnote/F50A7E0CD4CA463282B06E471097EA7A/7492)
...
![状态预览](https://note.youdao.com/yws/public/resource/6535547d10d104ba71d1bca109828f7c/xmlnote/C5717A723C4A4A9C87FA6CF8FB687364/7495)
...
![状态预览](https://note.youdao.com/yws/public/resource/6535547d10d104ba71d1bca109828f7c/xmlnote/544D60F5EAE14E658744B7CCCBCACEFC/7497)
...
&emsp;暂时没有很好的方法，可通过git fsck、git reflog等检查一致性、日志或其他第三方工具的方法来尝试恢复。
...
&emsp;保留工作区，并把暂存区（stage）和重置HEAD带来的差异也都放到工作区上，这是reset的默认模式

 - soft

&emsp;回退到特定版本，只回退commit信息，保留工作目录，并把重置 HEAD 所带来的新的差异放进暂存区（stage）
 
 - hard

&emsp;重置暂存区（stage）和工作区
...
## revert
### 撤销（反做）某个提交
&emsp;将某个提交进行反向操作然后产生一个revert commit message。
``` bash
...
## 变基（rebase）
&emsp;关于变基的说明看[文档](https://git-scm.com/book/zh/v2/Git-%E5%88%86%E6%94%AF-%E5%8F%98%E5%9F%BA)。

&emsp;变基的基本原则：多人协作时，拉取一个被变基然后强制推送的分支，要通知每个人执行再次变基（git pull --rebase）。
...
## 暂储（stash）
&emsp;stash在把当前的修改打包存储起来，然后执行其它操作时会很有用。
### 基本命令
``` bash
# 列出所有stash
git stash list
...
## reflog
&emsp;reflog == ref log，查看更多的log，reflog是属于本地的，可设定过期时间
``` bash
git reflog
...
## archive
&emsp;为仓库创建归档文件，如：.zip、tar.gz等文件。
``` bash
# 查看所有支持的格式
git archive -l
...
### pack
&emsp;Git 最初向磁盘中存储对象时所使用的格式被称为“松散（loose）”对象格式。 但是，Git 会时不时地将多个这些对象打包成一个称为“包文件（packfile）”的二进制文件，以节省空间和提高效率。 当版本库中有太多的松散对象，或者你手动执行 git gc 命令，或者你向远程服务器执行推送时，Git 都会这样做。 

&emsp;当文件是悬空（dangling）的时，不会将它们打包进新生成的包文件中。

&emsp;&lowast;.pack是包文件，&lowast;.idx是索引文件。

&emsp;详细的说明看[文档](https://git-scm.com/book/zh/v2/Git-%E5%86%85%E9%83%A8%E5%8E%9F%E7%90%86-%E5%8C%85%E6%96%87%E4%BB%B6)。
...
### hooks
&emsp;&lt;repso path&gt;/.git/hooks下面有各种钩子的example，修改脚本内容并把.sample后缀去掉就可以启用钩子了。具体钩子的用处和含义看[文档](https://git-scm.com/book/zh/v2/%E8%87%AA%E5%AE%9A%E4%B9%89-Git-Git-%E9%92%A9%E5%AD%90)。